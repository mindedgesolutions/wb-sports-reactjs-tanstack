import { AppBodyWrapper, AppTitleWrapper, SubmitBtn } from '@/components';
...
const SpaStadiumSingle = () => {
  ...
  const [pageTitle, setPageTitle] = useState('Add new stadium');
  ...
  useEffect(() => {
    if (data) {
      ...
      setResetKey((k) => k + 1);
      setPageTitle(fetching ? 'Fetching data' : data.name);
    } else {
      methods.reset({
        name: '',
        location: '',
      ...
      setPageTitle(id ? 'Fetching data' : 'Add new stadium');
    }
  }, [data]);
