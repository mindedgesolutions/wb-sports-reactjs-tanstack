      ...
      <Table className="text-[10px]">
        ...
        <TableBody>
          ...
          {!isLoading && data?.data?.length === 0 ? (
            ...
            data?.data?.map((data: IVocScheme, index) => (
              <TableRow
              ...
              >
                ...
                <TableCell>
                  <AppTooltip text={data.content} cropLen={100} />
                </TableCell>
                <TableCell>
                  <FormToggle
                    checked={data.is_active}
                    api={servicesApp.youthTraining.vocTraining.schemes.toggle(
                      Number(data.id),
                    )}
                ...
                <TableCell>
                  <span className="flex gap-6">
                    <Button
                      ...
                      size={'icon-xs'}
                      onClick={() => {
                        queryClient.setQueryData(['voc-scheme-selected'], data);
                      }}
                    >
                      <HiOutlinePencilAlt className="size-4 text-warn" />
                    </Button>
                    <AppDeleteModal
                      api={servicesApp.youthTraining.vocTraining.schemes.delete(
                        Number(data.id),
                      )}
                      queryKey="voc-schemes"
                      deleteQueryKey="voc-scheme-selected"
                      id={data.id}
                    />
                  </span>
                </TableCell>
